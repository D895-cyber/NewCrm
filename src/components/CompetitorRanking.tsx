    ...
    <Card className="ai8-card border-0">
      <CardHeader className="flex flex-row items-center justify-between pb-8">
        ...
        <button 
          ...
          onClick={() => {
            const reportData = [
              { brand: "AI8 Digital", mentions: 1247, rank: 1.8, share: "45%" },
              { brand: "Soroco", mentions: 892, rank: 2.4, share: "32%" },
              { brand: "Mixpanel", mentions: 756, rank: 2.8, share: "27%" },
              { brand: "Nintex", mentions: 634, rank: 3.2, share: "23%" },
              { brand: "Competitor E", mentions: 523, rank: 3.6, share: "19%" }
            ];
            const csvContent = convertToCSV(reportData);
            downloadCSV(csvContent, `competitor_ranking_report_${new Date().toISOString().split('T')[0]}.csv`);
            (window as any).showToast?.({
              type: 'success',
              title: 'Report Generated',
              message: 'Competitor ranking report exported to CSV file'
            });
          }}
        >
          View Full Report
