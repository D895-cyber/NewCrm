    ...
    <>
      ...
      <header className="bg-dark-bg border-b border-dark-color px-8 py-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-semibold text-dark-primary">Intelligence</h1>
            <p className="text-sm text-dark-secondary mt-1">AI model insights and competitive intelligence</p>
          </div>
                      <button 
              onClick={() => {
                const reportData = [
                  { metric: 'Total Mentions', value: totalMentionsData.reduce((sum, item) => sum + item.mentions, 0) },
                  { metric: 'Average Rank', value: averageRankData.reduce((sum, item) => sum + item.rank, 0) / averageRankData.length },
                  { metric: 'Top Brand', value: 'AI8 Digital' },
                ...
                (window as any).showToast?.({
                  ...
                  message: 'Intelligence report exported to CSV file'
                });
              }}
