...
export function WorkingPhotoCapture() {
  ...
  useEffect(() => {
    ...
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
      ...
      addDebugInfo('Camera API not supported');
    } else {
      addDebugInfo('Camera API supported');
      
      // Check permissions if available
      if (navigator.permissions) {
        navigator.permissions.query({ name: 'camera' as PermissionName })
          .then(result => {
            addDebugInfo(`Camera permission status: ${result.state}`);
            if (result.state === 'denied') {
              setCameraError('Camera permission denied. Please enable camera access in your browser settings.');
            }
          })
          .catch(err => addDebugInfo(`Permission check error: ${err.message}`));
      }
    }
  }, []);
  ...
  const startCamera = async () => {
    try {
      ...
      const cameraConfigs = [
        // Config 1: Back camera with standard resolution
        {
          video: { 
        ...
        // Config 2: Any camera with basic settings
        {
          video: { 
            width: { min: 640 },
            height: { min: 480 }
        ...
        // Config 3: Basic video (last resort)
        {
          video: true
        }
      ];
      ...
      while (!stream && configIndex < cameraConfigs.length) {
        try {
          addDebugInfo(`Trying camera config ${configIndex + 1}`);
  ...
  const uploadToCloudinary = async (photo: PhotoData) => {
    try {
      addDebugInfo(`Uploading ${photo.description} to Cloudinary...`);
      
      // Create FormData
      const formData = new FormData();
      formData.append('file', photo.file);
      formData.append('upload_preset', 'projectorcare'); // Use your Cloudinary upload preset
      formData.append('cloud_name', 'dxepnpgw7'); // Use your Cloudinary cloud name
      formData.append('folder', `fse-photos/${testVisitId}/${currentCategory}`);
      ...
      addDebugInfo(`Upload successful: ${result.secure_url}`);
  ...
  const testUpload = async () => {
    ...
    setIsUploading(true);
    setUploadProgress(0);
    addDebugInfo(`Starting upload of ${photos.length} photos to Cloudinary`);
    ...
    try {
      ...
      for (let i = 0; i < photos.length; i++) {
        ...
        try {
          ...
          setPhotos(prev => prev.map(p => 
            p.id === photo.id 
              ? { ...p, uploadStatus: 'success' as const, cloudinaryUrl }
              : p
          ));
          
          addDebugInfo(`Photo ${i + 1}/${photos.length} uploaded successfully`);
        } catch (error: any) {
          // Update photo status to error
          setPhotos(prev => prev.map(p => 
            p.id === photo.id 
              ? { ...p, uploadStatus: 'error' as const, error: error.message }
              : p
          ));
          
          addDebugInfo(`Photo ${i + 1}/${photos.length} upload failed: ${error.message}`);
        }
      ...
      addDebugInfo('All uploads completed');
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 space-y-6">
      ...
      <div className="bg-white rounded-lg p-4 shadow-sm">
        <h1 className="text-2xl font-bold text-gray-900 mb-2">📸 Working Photo Capture & Upload</h1>
        <p className="text-gray-600">Test camera integration and Cloudinary upload functionality</p>
      </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-3 text-sm text-gray-600">
          ...
          <div className="flex items-start gap-2">
            <div className="w-2 h-2 bg-blue-500 rounded-full mt-2 flex-shrink-0"></div>
            <p>Test upload to Cloudinary with real backend integration</p>
          </div>
          <div className="flex items-start gap-2">
            <div className="w-2 h-2 bg-blue-500 rounded-full mt-2 flex-shrink-0"></div>
